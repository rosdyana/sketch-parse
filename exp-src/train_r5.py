...
train_txt_list = jsonconfig["train_r5"]["train_txt_list"]
no_parts = jsonconfig["train_r5"]["no_parts"]
img_list = get_im_label(train_txt_list)

seg_weights = {}
for temp1 in range(len(train_txt_list)):
    global_stats_sum, global_stats_presence = find_med_frequency(read_txt_file(
        args['--LISTpath']+'train_' + train_txt_list[temp1] + '.txt'), no_parts[temp1])
...
# Init optimizer
optimizer_double = optim.SGD([{'params': get_1x_lr_params_NObn_double(model_double), 'lr': base_lr}, {'params': get_10x_lr_params_double(
    model_double), 'lr': 10*base_lr}, {'params': get_50x_lr_params_double(model_double), 'lr': 50*base_lr}], lr=base_lr, momentum=0.9)
...
for iter in range(maxIter+1):
    ...
    loss_double = loss_calc_seg(
        out_double[0], label[0], gpu0, seg_weights[selector])

    # do not iterate over pose output (last element in output list)
    for i in range(len(out_double)-2):
        loss_double = loss_double + \
            loss_calc_seg(out_double[i+1], label[i+1],
                          gpu0, seg_weights[selector])
    logseg = loss_double.data.cpu().numpy()
    print(iter, 'loss (Seg) = ', logseg)

    loss_double = loss_double + lambda1 * \
        loss_calc_pose(out_double[-1], pose[0], gpu0)
    ...
    optimizer_double = optim.SGD([{'params': get_1x_lr_params_NObn_double(model_double), 'lr': lr_}, {'params': get_10x_lr_params_double(
        model_double), 'lr': 10*lr_}, {'params': get_50x_lr_params_double(model_double), 'lr': 50*lr_}], lr=lr_, momentum=0.9)
    ...
    if iter % 1000 == 0 and iter != 0:
        snapPath = os.path.join(
            'data/snapshots', snapPrefix + str(iter) + '.pth')
