...
def get_iou(pred, gt, class_):
    ...
    max_label_dict = jsonconfig["table1"]["max_label_dict"]
...
model_A = getattr(resnet_dilated_frozen, 'Res_Deeplab')()  # TODO
model_B = getattr(resnet_dilated_frozen, 'Res_Deeplab')()  # TODO
model_C = getattr(resnet_dilated_frozen_pose, 'Res_Deeplab')()  # TODO
model_D = getattr(resnet_dilated_frozen_pose, 'Res_Deeplab')()  # TODO
...
prefix_A = 'r0_10000.pth'  # B_r1
prefix_B = 'r0_segnet2_12000.pth'  # BS_r1
prefix_C = 'DeepLab_RN_auxPose_L1_lr5e-4_SNL_SO5000.pth'  # change to BP_r1
prefix_D = 'DeepLab_RN_auxPose_L1_lr5e-4_SNL_SO5000.pth'  # BSP_r0
prefix_E = 'model_r5_p50x_D1_17000.pth'  # BSP_r5
    ...
    for class_selector in class_list:
        ...
        for i in img_list:
           ...
           # img = cv2.erode(img[:,:,0],kernel,iterations = 1)
            ...
            output_A = model_A(Variable(torch.from_numpy(
                img[np.newaxis, :].transpose(0, 3, 1, 2)).float(), volatile=True).cuda())
            output_B = model_B(Variable(torch.from_numpy(
                img[np.newaxis, :].transpose(0, 3, 1, 2)).float(), volatile=True).cuda())
            output_C = model_C(Variable(torch.from_numpy(
                img[np.newaxis, :].transpose(0, 3, 1, 2)).float(), volatile=True).cuda())
            output_D = model_D(Variable(torch.from_numpy(
                img[np.newaxis, :].transpose(0, 3, 1, 2)).float(), volatile=True).cuda())
            ...
            output_E = merge_parts(np.argmax(interp(output_E[3]).cpu(
            ).data[0].numpy().transpose(1, 2, 0), axis=2), selector)
